...
class iwac_Form_Field {
	...
	public function __construct( $post_id, $name ) {
		...
		$this->submitted_value = ( key_exists( $this->name, $_REQUEST ) ) ? $_REQUEST[$this->name] : null;

	}

	public function validate() {

		if ( !$this->submitted_value && $this->required )
			$this->_validationError( __( 'You left a required field <span>%1$s</strong> empty', 'iwacontact' ) );

		if ( 'email' == $this->validation && !preg_match( '/^[A-Z0-9._%-]+@[A-Z0-9._%-]+.[A-Z]{2,4}$/i', $this->submitted_value ) )
			$this->_validationError( __( 'Invalid email address provided for <span>%1$s</strong>', 'iwacontact' ) );

		return true;

	}

	private function _validationError( $error ) {
		throw new Exception( sprintf( $error, $this->title ), 1 );
	}
